...
async function serverSession(){
  ...
  if(r && r.ok){
    if(r.status) actStatus = r.status;
    if(r.active===false || r.status!=='running'){   // 没有进行中的活动 → 显示「等待下一场」页
      S.active=false; S.profile=r.profile||null; save(); return r;
    }
...
function custLanding(){ return (S.active===false) ? 'waiting' : 'home'; }   // 顾客落地:有进行中活动→大厅,没有→等待页
    ...
    if(c&&c.ok){
      ...
      if(MF.p2){
        ...
        if(S.loggedIn && !S.admin){ await serverSession();          // 刷新后重新水合状态
          const cur=(document.querySelector('.screen.active')||{}).id; if(cur==='screen-home'||cur==='screen-waiting') go(custLanding()); }
      }
...
/* ---------------- 路由 ---------------- */
function go(name){
  $('phone').classList.toggle('bare', name==='login'||name==='campaigns'||name==='admin'||name==='campdetail'||name==='waiting');   // 顶栏只在大厅显示
...
function spin(){
  if(spinning) return;
  if(!previewMode && homeBlock()){ statusModal(); return; }
...
$('waBtn') && $('waBtn').addEventListener('click', async ()=>{
  if(previewMode){ toastModal('👁 预览模式 —— 顾客在这里会生成下单码 + 跳去 WhatsApp 找你下单'); return; }
...
function renderWaiting(){
  const box=$('waitBody'); if(!box) return;
  const p=S.profile, nm=(p&&p.name)||S.name||'';
  let hist='';
  if(p && p.history && Object.keys(p.history).length){
    const items=Object.entries(p.history).sort((a,b)=>String(b[0]).localeCompare(String(a[0])));
    hist=items.map(([cid,h])=>`<div class="wt-item"><div class="wt-it">${h.title||cid}</div><div class="wt-in">🎁 抽中 <b>${h.prizeCount||0}</b> 件${(h.orders||0)>0?' · 🛒 已下单':''}</div></div>`).join('');
  }
  box.innerHTML=`
    <div class="wt-badge">★ 老顾客专属</div>
    <div class="wt-hi">嗨${nm?(' '+nm):''} 👋</div>
    ${hist?`<div class="wt-say">谢谢你参加我们的会员日!你的记录:</div><div class="wt-list">${hist}</div>`
          :`<div class="wt-say">目前没有进行中的活动。</div>`}
    <div class="wt-next"><div class="wt-ne">⏰</div><div class="wt-nt">敬请期待下一场 Member Day 或新活动!</div><div class="wt-ns">下一场我们会在 WhatsApp / 社群通知你 🔔</div></div>
    <button class="ghost" id="waitLogout">退出</button>`;
  const lo=$('waitLogout'); if(lo) lo.onclick=logout;
}
...
async function renderCampDetail(id){                            // 一个活动的完整后台(内容 + 数据 + 查顾客)
  ...
  MSG_RECOVER = cfg.msgRecover || DEF_MSG_RECOVER; MSG_ORDER = cfg.msgOrder || DEF_MSG_ORDER;   // 查顾客「复制给顾客」用这个活动的模板
  ...
  /* ---- 🎮 游戏预览(看回顾客当时玩的画面)---- */
  h+=`<div class="cd-sec">🎮 游戏预览</div>`;
  if(c.theme==='wheel'){
    h+=`<div class="adm-card"><button id="cdPreview" class="pv-btn">👁 浏览整个游戏</button><div class="adm-note">进去看到<b>完整的大转盘游戏</b> —— 能转、下面能选配套,和顾客玩的一模一样(预览,不影响真实数据)。</div></div>`;
  } else {
    h+=`<div class="adm-card"><div class="adm-empty">这个游戏(${THEME_LABEL[c.theme]||c.theme})还没做好 —— 第3步做好后这里能预览。</div></div>`;
  }
  ...
  if(data){
    ...
    if(c.status==='running'){   // 客服操作(只在进行中的活动有意义)
      h+=`<div class="adm-card"><div class="adm-h">🧾 核对下单码</div>
        <div class="code-add"><input id="csCode" placeholder="顾客的码 MD-2B-123456" autocapitalize="characters"><button id="csBtn">核对</button></div>
        <div id="csResult"></div>
        <div class="adm-note">顾客发来的下单码 → 核对配套/好礼/价格(以服务器为准,防改文字)。</div></div>`;
      h+=`<div class="adm-card"><div class="adm-h">📋 待跟进名单</div>
        <button id="leadsBtn" class="lead-btn">⬇️ 导出 CSV(玩过没下单)</button>
        <div class="adm-note">导出玩过但没下单的顾客 → WhatsApp 群发提醒。</div></div>`;
    }
  }
  ...
  /* ---- 🎛 控制 ---- */
  if(c.status==='running'){
    const paused = c.serveStatus==='paused';
    h+=`<div class="cd-sec">🎛 控制</div><div class="adm-card">
      <button id="cdPause" class="pause-btn">${paused?'▶️ 恢复活动':'⏸ 暂停活动'}</button>
      <button id="cdEnd" class="danger-btn" style="margin-top:9px">🔚 结束这个活动并归档</button>
      <div class="adm-note">暂停 = 顾客暂时不能玩(已中好礼仍可 24h 内兑换);结束 = 永久归档,不可逆。</div></div>`;
  ...
  box.innerHTML=h;
  const pvb=$('cdPreview'); if(pvb) pvb.onclick=enterPreview;   // 👁 浏览整个游戏
  ...
  const cv=$('csBtn'); if(cv) cv.onclick=csVerify;
  const lb2=$('leadsBtn'); if(lb2) lb2.onclick=exportLeads;
  const pb=$('cdPause'); if(pb) pb.onclick=async ()=>{ const ns=(c.serveStatus==='paused')?'running':'paused'; const rr=await apiPOST('/api/admin',{action:'setStatus',status:ns}); if(rr&&rr.ok){ actStatus=ns; saveStatus(); toastModal(ns==='paused'?'⏸ 已暂停':'▶️ 已恢复'); renderCampDetail(id); } else toastModal('失败'); };
...
$('loginBtn').onclick=async ()=>{
  ...
  save();
  go(S.admin?'campaigns':custLanding());
};
function logout(){
  if(previewMode){ exitPreview(); return; }   // 预览模式:退出=回后台
...
/* ---------------- 启动 ---------------- */
if(S.loggedIn) go(S.admin?'campaigns':custLanding()); else go('login');
