...
async function renderCampDetail(id){                            // 一个活动的完整后台(内容 + 数据 + 查顾客)
  ...
  if(c.theme==='wheel'){
    h+=`<div class="pv-card"><div class="wheel-wrap pv-wheel"><div class="wheel-pointer"></div><div class="wheel" id="pvWheel"></div><div class="wheel-hub">🎡</div></div>
      <div class="pv-note">👆 顾客当时玩的大转盘(预览,只看不能转)</div></div>`;
  } else {
  ...
  box.innerHTML=h;
  if(c.theme==='wheel' && $('pvWheel')) buildWheel($('pvWheel'));   // 渲染预览转盘(静态)
